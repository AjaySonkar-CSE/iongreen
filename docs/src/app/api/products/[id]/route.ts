...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const query = `UPDATE products SET name = ?, slug = ?, description = ?, features = ?, specifications = ?, applications = ?, benefits = ?, image_url = ?, category = ?, gallery = ?, is_active = ?, is_featured = ?, updated_at = NOW() WHERE id = ?`;
